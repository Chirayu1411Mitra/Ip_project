...
function DoubtPage() {
  ...
  const totalAnswers = doubts.reduce((sum, d) => sum + (d.answers?.length ?? 0), 0);
    ...
    <div className="flex-1 overflow-y-auto bg-gray-50 min-h-screen">
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">Ask doubts, give answers, help your peers</p>
        </div>

        {/* Stats row */}
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            { label: "Total Doubts",   value: doubts.length,                         icon: <HelpCircle size={16} />,    color: "#7c3aed" },
            { label: "Total Answers",  value: totalAnswers,                           icon: <MessageSquare size={16} />, color: "#059669" },
            { label: "Unanswered",     value: doubts.filter(d=>!d.answers?.length).length, icon: <TrendingUp size={16} />,    color: "#d97706" },
