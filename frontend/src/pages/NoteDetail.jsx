...
const NoteDetail = () => {
  ...
  return (
    // Added padding for mobile (px-4) and adjusted top spacing (py-6)
    <div className="space-y-4 sm:space-y-6 max-w-5xl mx-auto px-4 sm:px-6 py-6">
      
      {/* Back Button */}
      <button
        id="note-detail-back-btn"
        onClick={() => navigate("/notes")}
        className="flex items-center gap-2 text-gray-500 hover:text-[#7C3AED] font-medium text-sm transition-colors w-fit"
      >
        <ArrowLeft size={18} />
        Back to Notes
      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl sm:rounded-[28px] border border-gray-100 overflow-hidden shadow-sm">
            {/* Adjusted padding and truncation for smaller screens */}
            <div className="bg-gradient-to-r from-[#7C3AED] to-[#9F67FF] px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-3">
                <FileText className="w-4 h-4 sm:w-5 sm:h-5 text-white shrink-0" />
                <span className="text-white font-semibold text-xs sm:text-sm truncate max-w-[150px] sm:max-w-[300px]">
                  {note.fileName}
                </span>
              </div>
              <button
                ...
                className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-white/20 hover:bg-white/30 text-white rounded-xl text-xs sm:text-sm font-medium transition-all backdrop-blur-sm shrink-0"
              >
                <Download size={16} />
            ...
            {/* PDF iframe - adjusted height for mobile */}
            {note.downloadUrl ? (
              <iframe
                src={note.downloadUrl}
                title={note.title}
                className="w-full border-0 h-[400px] sm:h-[600px]"
              />
            ) : (
        ...
        <div className="space-y-4 sm:space-y-6">
          {/* Note Info Card */}
          <div className="bg-white rounded-2xl sm:rounded-[28px] border border-gray-100 p-5 sm:p-6 shadow-sm">
          ...
          <button
            ...
            onClick={handleDownload}
            className="w-full flex items-center justify-center gap-2 py-3.5 sm:py-4 bg-gradient-to-r from-[#7C3AED] to-[#9F67FF] text-white rounded-2xl font-bold text-sm shadow-lg shadow-purple-200 hover:shadow-xl hover:shadow-purple-300 active:scale-[0.97] transition-all duration-200"
          >
            <Download size={18} />
            Download PDF
          </button>
